...
if (isset($_POST['add_patient'])) {
    ...
    if ($firstname === '' || !preg_match($namePattern, $firstname)) { echo json_encode(['status' => 'error', 'message' => 'Invalid First Name']); exit; }
    if ($middlename !== '' && !preg_match($namePattern, $middlename)) { echo json_encode(['status' => 'error', 'message' => 'Invalid Middle Name']); exit; }
    if ($lastname === '' || !preg_match($namePattern, $lastname)) { echo json_encode(['status' => 'error', 'message' => 'Invalid Last Name']); exit; }
    if ($sex !== 'Male' && $sex !== 'Female') { echo json_encode(['status' => 'error', 'message' => 'Invalid Sex Value']); exit; }
    ...
    $today = new DateTime();
    $minDate = new DateTime('1900-01-01'); // ✅ Oldest reasonable birth year

    // Check if DOB is in the future
    if ($dob_dt > $today) { 
        echo json_encode(['status' => 'error', 'message' => 'Birthday cannot be in the future']); 
        exit; 
    }

    // Check if DOB is before 1900
    if ($dob_dt < $minDate) { 
        echo json_encode(['status' => 'error', 'message' => 'Invalid birth year. Please enter a valid date after 1900.']); 
        exit; 
    }

    if ($address === '') { echo json_encode(['status' => 'error', 'message' => 'Address is Required']); exit;  }

    $phoneNum = preg_replace('/\D/', '', $phone);
    if (strlen($phoneNum) !== 11) { echo json_encode(['status' => 'error', 'message' => 'Phone Number must be 11 Digits Only']); exit; }
    $phone = $phoneNum;
    ...
    $countQuery = mysqli_query($con,
        ...
        WHERE YEAR(patient_added) = YEAR(CURDATE())"
    );
    $countRow = mysqli_fetch_assoc($countQuery);
    $yearlyCount = (int)$countRow['count_year'];
    $nextCount = $yearlyCount + 1;

    // Combine into patient code
    $patient_code = $year . '-' . $nextCount;

    // --- INSERT PATIENT INFO ---
    try {
        $patient_added = date('Y-m-d H:i:s');
        $stmt = mysqli_prepare($con, "INSERT INTO patient_info_tbl 
            (patient_code, patient_fname, patient_mname, patient_lname, patient_sex, patient_home_add, patient_dob, patient_phone, patient_added) 
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)");
        mysqli_stmt_bind_param($stmt, 'sssssssss', $patient_code, $firstname, $middlename, $lastname, $sex, $address, $dob, $phone, $patient_added);
    ...
    if ($selected_package_id) {
        ...
        foreach ($_POST['selected_services'] as $svc_id_raw) {
            ...
            if (isset($group_service_stmt)){
                mysqli_stmt_close($group_service_stmt);
            }
    ...
    //         $final_total,   // d - pro-rate of discount
    //         $custom_discount,  // d
    //         $curr_time         // s
    //     );
...
<div class="page-title">
    <div class="title">Create New Patient</div>
</div>

<!-- ======= ADD PATIENT CARD ======= -->
<div class="add-account-card">
    <form class="add-account-form" method="POST" id="addPatientForm">
        ...
        <div class="form-section">
            ...
            <div class="form-row">
                <div class="form-group">
                    ...
                    <input type="text" name="firstname" id="firstname" placeholder="e.g. John" required>
                </div>
                <div class="form-group">
                    <label for="middlename">Middle Name</label>
                    <input type="text" name="middlename" id="middlename" placeholder="e.g. Matthew">
                </div>
                <div class="form-group">
                    <label for="lastname">Last Name</label>
                    <input type="text" name="lastname" id="lastname" placeholder="e.g. Reyes" required>
                </div>
            </div>
            <div class="form-row">
                <div class="form-group">
            ...
            <div class="form-row">
                <div class="form-group" style="flex:2;">
                    ...
                    <input type="text" name="address" id="address" placeholder="Street, City" required>
                </div>
                <div class="form-group">
                    <label for="phone">Contact Number</label>
                    <input type="tel" name="phone" id="phone" placeholder="09123456789" pattern="[0-9]{11}" required>
                </div>
            </div>
        </div>
        ...
        <div class="form-actions">
            <a href="#" id="cancelAddPatient" class="btn btn-outline btn-sm">
